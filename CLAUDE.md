...
- **Validate external resources before `session.add()`** — since a bare `return` inside `session_scope` commits, always resolve and validate channels/config _before_ writing rows. Write-then-validate risks orphaned DB rows on early return (e.g. channel invalid after order already flushed).
- **`guild.get_channel()` does not resolve threads** — use `guild.get_channel(id) or guild.get_thread(id)` when a stored channel ID might refer to a thread. `get_channel()` returns `None` for thread IDs, which can silently delete data (e.g. reminder cleanup).
- **`isinstance()` breaks mock dispatch in tests** — use `hasattr(source, "attr")` for duck-typed branching between discord types, then `cast()` inside each branch so Pylance narrows the type. `isinstance(MagicMock(), Context)` is always `False`.
- **Test mocks need both `guild` and `guild_id`** — when production code uses `interaction.guild_id` (the cached scalar), test mocks must set it explicitly: `mock.guild_id = mock.guild.id`. Setting only `guild` leaves `guild_id` as a new auto-generated `MagicMock`, causing SQLAlchemy binding errors.
- **Use `==` not `is` for `Enum.value` comparisons** — `is` checks identity, not equality. String interning makes `is` accidentally work in simple cases but it can silently fail across module boundaries.
