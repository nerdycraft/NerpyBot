...
- **`model_fields_set` for nullable PATCH fields** — a field with `str | None = None` makes `None` ambiguous: it means both "client omitted the field" and "client explicitly cleared it". `if body.field is not None` cannot distinguish these; use `if "field_name" in body.model_fields_set` to check whether the client sent the field at all, then assign `body.field` (already normalized) directly. This applies more strongly when a `BeforeValidator` normalizes `""` → `None`, since the normalized value is also `None`.
